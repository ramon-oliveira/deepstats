...
try:
    df = pd.read_csv('mnist_results/bayesian_uncertainty_with_unknown.csv')
except:
    df = pd.DataFrame()
...
for experiment, network, dataset, inside_labels, unknown_labels in params:
    ...
    df = df.append(out)
    df.to_csv('mnist_results/bayesian_uncertainty_with_unknown.csv', index=False)
    gc.collect()

# Without unknown labels in the training set

try:
    df = pd.read_csv('mnist_results/bayesian_uncertainty_without_unknown.csv')
except:
    df = pd.DataFrame()
...
for experiment, network, dataset, inside_labels, unknown_labels in params:
    ...
    df = df.append(out)
    df.to_csv('mnist_results/bayesian_uncertainty_without_unknown.csv', index=False)
    gc.collect()
